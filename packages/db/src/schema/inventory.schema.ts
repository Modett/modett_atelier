...
export const productVariants = inventory.table(
  'product_variants',
  {
  ...
  (t) => [
    unique('uq_variant_product_color_size').on(
      t.product_id,
      t.color,
      t.size,
    ),
...
export const variantStock = inventory.table(
  'variant_stock',
  {
    ...
    available_qty: integer('available_qty').generatedAlwaysAs(
      sql`(in_stock_qty - held_qty)`,
    ),
  ...
  (t) => [
    check(
      'chk_held_not_exceed_stock',
      sql`${t.held_qty} <= ${t.in_stock_qty}`,
    ),
  ],
)
...
export const inventoryReconciliationLog = inventory.table(
  'inventory_reconciliation_log',
  {
    ...
    delta: integer('delta').generatedAlwaysAs(
      sql`(actual_count - aggregate_count)`,
    ),
