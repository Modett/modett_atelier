...
export const cartItems = cart.table(
  'cart_items',
  {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const reservations = cart.table(
  ...
  (t) => [
    check(
      'chk_grace_set_together',
      sql`((${t.payment_submitted_at} IS NULL AND ${t.grace_expires_at} IS NULL) OR (${t.payment_submitted_at} IS NOT NULL AND ${t.grace_expires_at} IS NOT NULL))`,
    ),
  ],
)
