 ...
 * loyalty_ledger: IMMUTABLE — append only. Never UPDATE or DELETE rows.
...
export const loyaltyAccounts = loyalty.table(
  'loyalty_accounts',
  {
  ...
  (t) => [
    ...
    check('chk_loyalty_lifetime_earned_non_neg', sql`${t.lifetime_earned} >= 0`),
  ],
)
...
export const loyaltyRules = loyalty.table('loyalty_rules', {
  ...
  redemption_rate_by_currency_json: jsonb(
  ...
  max_redeem_percent: numeric('max_redeem_percent', {
    precision: 5,
    scale: 2,
  })
    .notNull()
...
export type NewLoyaltyAccount = InferInsertModel<typeof loyaltyAccounts>
export type LoyaltyLedgerRow = InferSelectModel<typeof loyaltyLedger>
export type NewLoyaltyLedgerRow = InferInsertModel<typeof loyaltyLedger>
...
export type NewLoyaltyGrant = InferInsertModel<typeof loyaltyGrants>
