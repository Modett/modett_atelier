...
export async function getVariantsWithStock({
  ...
  `)
  return (rows.rows as unknown as VariantWithStockRow[]) ?? []
}
...
export async function getActiveStylingGuide({
  ...
  const rows = await db
    ...
    .where(
      and(
        eq(productStylingGuides.productId, productId),
        eq(productStylingGuides.active, true),
      ),
    )
  return rows[0] ?? null
}
...
export async function getProductById({
  ...
  id: string
}): Promise<(Product & { prices: ProductPrice; images: ProductImage[] }) | null> {
  const productRows = await db
    .select()
    .from(products)
...
export async function createProduct({
  ...
  return await db.transaction(async (tx) => {
    ...
    const [priceRow] = await tx
      ...
      .returning()
    if (!priceRow) throw new Error('createProduct: price insert failed')
    return { ...product, prices: priceRow } as Product & { prices: ProductPrice }
  })
}
...
export async function updateProduct({
  ...
  const { lkrAmount, sgdAmount, usdAmount, ...productData } = data as {
    lkrAmount?: string
    sgdAmount?: string
    usdAmount?: string
    [k: string]: unknown
  ...
  await db.transaction(async (tx) => {
    const updatePayload: Record<string, unknown> = { ...productData, updatedAt: new Date() }
    if (Object.keys(updatePayload).length > 1) {
      await tx.update(products).set(updatePayload as Partial<Product>).where(eq(products.id, id))
    }
    if (
      lkrAmount !== undefined ||
      sgdAmount !== undefined ||
      ...
      const priceUpdate: Record<string, unknown> = { updatedAt: new Date() }
      if (lkrAmount !== undefined) priceUpdate.lkrAmount = lkrAmount
      if (sgdAmount !== undefined) priceUpdate.sgdAmount = sgdAmount
      if (usdAmount !== undefined) priceUpdate.usdAmount = usdAmount
      await tx
        .update(productPrices)
        .set(priceUpdate as Partial<ProductPrice>)
        .where(eq(productPrices.productId, id))
    }
  })
...
export async function softDeleteProduct({ id }: { id: string }): Promise<void> {
  await db
    .update(products)
...
export async function createProductImage({
  ...
  const [row] = await db
    ...
    .returning()
  if (!row) throw new Error('createProductImage: no row returned')
  return row
}
...
export async function createVariant({
  ...
  return await db.transaction(async (tx) => {
    const [variant] = await tx
      ...
      .returning()
    if (!variant) throw new Error('createVariant: no row returned')
...
export async function upsertStylingGuide({
  productId,
  type,
  linkUrl,
  contentJson,
  ...
  if (existing[0]) {
    const [updated] = await db
      ...
      .where(eq(productStylingGuides.productId, productId))
      .returning()
    if (!updated) throw new Error('upsertStylingGuide: update failed')
    return updated
  }
  const [inserted] = await db
    ...
    .returning()
  if (!inserted) throw new Error('upsertStylingGuide: insert failed')
  return inserted
}

export async function getBestsellerList(): Promise<
  Array<BestsellerEntry & ProductListItemRow>
> {
  const rows = await db.execute(sql`
    SELECT
      bl.id,
  ...
  `)
  return (rows.rows as Array<BestsellerEntry & Omit<ProductListItemRow, 'id'>>) ?? []
}

export async function addToBestsellerList({
  productId,
  sortOrder = 0,
  addedByAdminId,
}: {
  ...
  const [row] = await db
    ...
    .returning()
  if (!row) throw new Error('addToBestsellerList: no row returned')
  return row
}

export async function removeFromBestsellerList({
  productId,
}: {
  productId: string
}): Promise<void> {
...
export async function reorderBestsellerList({
  orderedProductIds,
}: {
...
export async function createBanner({
  ...
  const [row] = await db
    ...
    .returning()
  if (!row) throw new Error('createBanner: no row returned')
  return row
}
