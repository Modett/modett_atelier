 ...
 */
import { pgSchema, uuid, text, boolean, integer, timestamp, jsonb, unique, check, } from 'drizzle-orm/pg-core';
...
export const ordersTable = orders.table('orders', {
    id: uuid('id').primaryKey().defaultRandom(),
    ...
    shipping_method_id: uuid('shipping_method_id').references(() => shippingMethods.id, { onDelete: 'set null' }),
    ...
    check('chk_order_identity', sql `(${t.user_id} IS NOT NULL OR ${t.guest_email} IS NOT NULL)`),
]);
export const promoRedemptions = orders.table('promo_redemptions', {
    id: uuid('id').primaryKey().defaultRandom(),
...
export const orderItems = orders.table('order_items', {
    ...
    unit_price_snapshot_currency: currencyCodeEnum('unit_price_snapshot_currency').notNull(),
    tax_amount: text('tax_amount').notNull().default('0'),
...
export const orderAddresses = orders.table('order_addresses', {
    id: uuid('id').primaryKey().defaultRandom(),
    order_id: uuid('order_id')
        .notNull()
        .references(() => ordersTable.id, { onDelete: 'cascade' }),
